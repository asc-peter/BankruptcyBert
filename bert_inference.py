...
bert_final_list.write('CIK' + '\t' + 'REPORT PERIOD END DATE' + '\t' + 'FILE DATE' + '\t' + 'FORM TYPE' + '\t' + 'POS' + '\t' + 'NEG' + '\t' + 'NEU' + '\n')
...
for d in dir:
    for txt in os.listdir(d):
        if '.txt' in txt and 'download' not in txt and 'DOWNLOADLOG' not in txt and 'newfile' not in txt and 'temp' not in txt and '._' not in txt:
                                    ...
                                    if t['input_ids'] != []: 
                                        if t['input_ids'].shape[0] <= 16:
                                            ...
                                            for i in range(nem):
                                                ...
                                                tak_batch['token_type_ids'] = t['token_type_ids'][i*16: (i+1)*16].cuda()
                                                tak_batch['attention_mask'] = t['attention_mask'][i*16: (i+1)*16].cuda()
                                            ...
                                            tak_batch['token_type_ids'] = t['token_type_ids'][nem*16: nem*16+mec].cuda()
                                            tak_batch['attention_mask'] = t['attention_mask'][nem*16: nem*16+mec].cuda()
                                            output = model(**tak_batch)
                                            logits = output.logits
                                            logits = F.softmax(logits, dim = -1)
                                            sums = torch.sum(logits, dim=0).tolist()
                 ...
                 var_dic['neu'] = str(neu)    
                 try:
                     bert_final_list.write(var_dic['CIK'] + '\t' + var_dic['REPORT PERIOD END DATE'] + '\t' + var_dic['FILE DATE'] + '\t' + var_dic['FORM TYPE'] + '\t' + var_dic['pos'] + '\t' + var_dic['neg'] + '\t' + var_dic['neu'] + '\n')
                 except:
